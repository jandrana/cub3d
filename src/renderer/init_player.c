...
static void find_player_coord(t_map *map, unsigned int player_coord[2])
{
    ...
    while (row < map->rows)
    {
        ...
        while (col < map->cols)
        {
            if (map->mt[row][col] == PLAYER_NORTH || map->mt[row][col] == PLAYER_SOUTH || map->mt[row][col] == PLAYER_EAST || map->mt[row][col] == PLAYER_WEST)
            {
                player_coord[0] = row;
                player_coord[1] = col;
                map->mt[row][col] = 0;
...
void	init_player(t_game *game)
{
    ...
    game->map->player.x = player_coord[0] + 0.5;
    game->map->player.y = player_coord[1] + 0.5;
    game->map->player.angle = 0.0;
    game->map->player.speed = 0.0;
}
